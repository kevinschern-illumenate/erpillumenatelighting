"""Generator for ilL-Webflow-Product.csv — Webflow product entries for tape/neon.

Multi-line for configurator_options and attribute_links child tables.
Reuses the same CSV filename but with tape/neon-specific content and
product type set to "Tape Neon Template".
"""
...
def generate(config: FixtureBuilderConfig, output_dir: str) -> str:
    """Generate ilL-Webflow-Product.csv for tape/neon and return the filepath."""
    rows = []
    wf = config.tape_neon_webflow

    for tmpl in config.tape_neon_templates:
        product_name = tmpl.template_name or f"{config.series_name} {tmpl.product_category}"
        ...
        primary[2] = "Tape Neon Template"
        primary[3] = wf.product_category
        primary[4] = tmpl.series or config.series_name
        primary[5] = 1   # Is Active
        primary[6] = tmpl.template_code  # Fixture Template (reused field)
        primary[11] = tmpl.default_tape_spec  # LED Tape Spec
