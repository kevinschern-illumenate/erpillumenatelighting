...
def validate_and_quote_multisegment(
	...
	Args:
		...
		override_max_run_ft: Optional user-supplied max run length (ft) that
			replaces the computed effective max run for the fixture as a
			whole. Applies across all segments/runs, not per-segment — if the
			fixture's total tape length exceeds it, a new run (with its own
			leader cable and power supply output) is still required.
...
def _compute_multisegment_outputs(
	...
	Args:
		...
		override_max_run_ft: Optional user-supplied max run length (ft) that
			replaces the computed effective max run for the fixture as a
			whole (applies across all segments/runs, not per-segment).
	...
	# for the fixture as a whole (not per-segment). If a multi-segment fixture
	# exceeds this length, it needs a new run with its own leader cable/power
	# supply output, exactly as it would with the computed effective max.
...
def _create_or_update_multisegment_fixture(
	...
	# as a whole across all segments/runs, not per-segment)
	if override_max_run_ft is not None and override_max_run_ft > 0:
