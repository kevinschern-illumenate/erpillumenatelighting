...
frappe.listview_settings["ilL-Webflow-Product"] = {
	add_fields: ["sync_status", "is_active"],

	onload(listview) {
		// "Targeted to Brand" — server-side filter via custom action that
		// rewrites the listview filter using the target_brands child table.
						...
						primary_action(values) {
							frappe.call({
								...
								callback(resp) {
									...
									if (!names.length) {
										frappe.msgprint(
											__("No products targeted to {0}.", [values.brand])
										);
										return;
									}
